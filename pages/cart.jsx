...
const Cart = () => {
  ...
  const createOrder = async (data) => {
    try {
      const res = await axios.post("http://trpizzawebapp/api/orders", data);
      if (res.status === 201) {
        dispatch(reset());
  ...
  const ButtonWrapper = ({ currency, showSpinner }) => {
    ...
    }, [currency, showSpinner, dispatch, options]); // Include dispatch and options in the dependency array
